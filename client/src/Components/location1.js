...
export default function Location() {
  ...
  useEffect(async () => {
    await axios
      .get("http://localhost:5000/eventRegister")
      .then((res) => {
        setGetData(res.data);
  ...
  const handleSubmit = async (e, values) => {
    ...
    if (+new Date(cur) <= +new Date(sta) && +new Date(sta) <= +new Date(en)) {
      if (action === "") {
        await axios
          .post("http://localhost:5000/eventRegister", obj)
          .then((res) => Success())
          .catch((err) => console.log(err));
        clear();
      } else {
        await axios
          .put(`http://localhost:5000/eventRegister/${action}`, obj)
          .then((res) => Update())
          .catch((err) => console.log(err));
        clear();
      }
  ...
  const edits = async (item) => {
    ...
    setAction(item);
    await axios
      .get(`http://localhost:5000/eventRegister/${item}`)
      .then((res) => {
  ...
  const deletes = async (item) => {
    swal({
      ...
      if (response) {
        await axios
          .delete(`http://localhost:5000/eventRegister/${item}`)
          .then(() => {
            setLoad(!load);
